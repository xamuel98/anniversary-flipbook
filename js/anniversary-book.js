// Note: Page content is now loaded from embedded-pages.js (window.embeddedPages)
...
function updateDepth(book, newPage) {
	...
	if (newPage > 3)
		$(".anniversary-book .p2 .depth").css({
			width: depthWidth,
			left: 20 - depthWidth,
		});
	else $(".anniversary-book .p2 .depth").css({ width: 0 });

	depthWidth = 16 * Math.min(1, ((pages - page) * 2) / pages);

	if (newPage < pages - 3)
		$(".anniversary-book .p27 .depth").css({
			width: depthWidth,
			right: 20 - depthWidth,
		});
	else $(".anniversary-book .p27 .depth").css({ width: 0 });
}
...
function loadPage(page) {
	fetch("pages/page" + page + ".html")
		...
		.then((pageHtml) => {
			$(".anniversary-book .p" + page).html(
				pageHtml.replace("anniversary-flipbook/", "")
			);
		})
		.catch((error) => {
...
function zoomHandle(e) {
	if ($(".anniversary-book").data().zoomIn) zoomOut();
...
function zoomThis(pic) {
	var position,
		...
		completeTransition = function () {
			...
			if ($(".anniversary-book").data().zoomIn) {
	...
	$(".anniversary-book").data().zoomIn = true;

	$(".anniversary-book").turn("disable", true);
	...
	tmpPic.load(function () {
		var realWidth = $(this)[0].width,
			...
			picPosition = {
				top:
					(picPos.top - zCenterY) * zoomFactor +
					zCenterY +
					bookPos.top,
				left:
					(picPos.left - zCenterX) * zoomFactor +
					zCenterX +
					bookPos.left,
			};
...
function zoomOut() {
	var transitionEnd = $.cssTransitionEnd(),
		completeTransition = function (e) {
			$("#book-zoom").unbind(transitionEnd);
			$(".anniversary-book").turn("disable", false);
			$("body").css({ overflow: "auto" });
			moveBar(false);
		};

	$(".anniversary-book").data().zoomIn = false;
